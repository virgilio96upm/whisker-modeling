...
OUTPUT_CSV_FILE = "recorded_data_30102023.csv"
...
RY_range = np.linspace(-0.350, 0.350, 30)
RX_range = np.linspace(-0.350, 0.350, 30)
    ...
    while True:
        ...
        if j < num_samples:
            ...
            if move_completed and state.output_int_register_0 == 1:
                ...
                # Send position
                setp.__dict__["input_double_register_%i" % 3] = rx[j]
                setp.__dict__["input_double_register_%i" % 4] = ry[j]
                con.send(setp)
                ...
                # measure sensor
                sensor_measured = read_sensor()
                data_recorded.writerow([rx[j],ry[j],sensor_measured[0],sensor_measured[1],sensor_measured[2]])
                #next position
                j+=1
                watchdog.input_int_register_0 = 0
